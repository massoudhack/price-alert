...
def get_interval(sym, atype, cur, tgt):
    """
    جدول ثابت بر اساس فاصله پیپ:

    طلا (XAUUSD / XAGUSD):
      > 200 pip  → 180s  (3 دقیقه)
      100–200    → 120s  (2 دقیقه)
      30–100     → 60s   (1 دقیقه)
      < 30       → 30s   (30 ثانیه)

    فارکس:
      > 200 pip  → 7200s (2 ساعت)
      100–200    → 3600s (1 ساعت)
      50–100     → 1800s (30 دقیقه)
      10–50      → 900s  (15 دقیقه)
      5–10       → 120s  (2 دقیقه)
      < 5        → 60s   (1 دقیقه)
    ...
    if is_gold:
        if pips > 200: return 180
        if pips > 100: return 120
        if pips > 30:  return 60
        return 30

    if pips > 200: return 7200
    if pips > 100: return 3600
    if pips > 50:  return 1800
    if pips > 10:  return 900
    if pips > 5:   return 120
    return 60
